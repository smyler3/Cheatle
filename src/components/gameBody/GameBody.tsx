...
export default function GameBody() {
    const { apiVersion, board, maxPossibleScore } = useFetchedData();
    ...
    const [incorrectBannerType, setIncorrectBannerType] =
        useState<GuessErrorsType | null>(null);
    ...
    }, [openResultModal, gameState, apiVersion]);
    ...
    const handleSubmit = (currentGuess: CurrentGuessType) => {
        ...
        if (isRepeat || isTooSmall) {
            ...
            setIncorrectBannerType(
                isRepeat ? GUESS_ERRORS.DUPLICATE : GUESS_ERRORS.TOO_SMALL,
            );
        }
        ...
        if (!isRepeat && !isTooSmall && isValid) {
            ...
            setValidWordsMap((prev) => {
                ...
                const newWord: WordSubset = {
                    ...oldWord,
                    revealedText: currentText,
                    isGuessed: true,
                };

                const updatedInnerWordMap: Map<string, WordSubset> = new Map(
                    innerWordMap.set(currentText, newWord),
                );
                const updatedValidWords: ValidWordsMap = new Map(prev).set(
                    currentValue,
                    updatedInnerWordMap,
                );
    ...
    const handleTileSelect = (tile: TileType, selectedPosition: number) => {
        const prevPosition: number | undefined =
            currentGuess.prevTileOrder.at(-1);
        ...
        // Adding to guess
            ...
            ADJACENT_LIST[prevPosition].includes(selectedPosition)
        ) {
            setCurrentGuess((prev) => ({
    ...
    return (
        <main className={styles.gameBodyMain}>
            {createPortal(<ModalManager />, document.body)}
            <CountdownTimer />
            <IncorrectDetailsBanner
                key={incorrectGuessCount}
                showType={incorrectBannerType}
                setShowType={setIncorrectBannerType}
            />
            <GameBoard>
                {board.map((tile, index) => {
                    return (
                        <Tile
                            ...
                            position={index}
                            lastGuess={lastGuess}
                            clearLastGuess={clearLastGuess}
                            selectedTiles={currentGuess.prevTilePositions}
                            handleClick={handleTileSelect}
                        />
                    );
                })}
            </GameBoard>
            ...
            />
            <GuessList shouldShowScore={true} />
        </main>
    );
}
